...
bool gm_graph::load_binary_internal(FILE*f, uint32_t magic_word, bool need_semi_sort) 
{
    ...
    if (i != 1) {
        ...
        if (key2 == (int32_t) magic_word) {
            ...
            fprintf(stderr, "wrong file format, KEY mismatch: %zd, %08x, expected:%08x\n", i, key, magic_word);
            goto error_return;
        }
    }
...
#define BITS_TO_EDGE(X) ((old_flipped_format)? (X) : (saved_edge_t_size == 4) ? n32tohedge(X) : n64tohedge(X))
    ...
    for (node_t i = 0; i < N + 1; i++) {
        edge_t key;
        size_t k = fread(&key, saved_edge_t_size, 1, f);
    ...
    for (edge_t i = 0; i < M; i++) {
        node_t key;
        size_t k = fread(&key, saved_node_t_size, 1, f);
        ...
        )
{
    ...
    //---------------------------------------
    num_node_props = (int32_t)vprop_schema.size();
    ...
    //---------------------------------------
    num_edge_props = (int32_t)eprop_schema.size();
